...
int SizeRecursive(const BinaryTreeNode* tree_node) {
  ...
  return 1 + SizeRecursive(tree_node->right_child) 
           + SizeRecursive(tree_node->left_child); 
}
...
bool InsertRecursive(BinaryTreeNode* &tree_node, int value) {
  if(!tree_node) {
    tree_node = new BinaryTreeNode{value, nullptr, nullptr}; 
    return true;
  }

  if(tree_node->data == value) return false;

  auto trg = (tree_node->data > value) 
    ? tree_node->left_child 
    : tree_node->right_child;

  return InsertRecursive(trg, value);
}
...
// Delete all helper, courtesy of the binary_tree_test.cc file
...
bool BinaryTree::Insert(int value) { return InsertRecursive(head, value); }
...
bool BinaryTree::Remove(int value) { return RemoveRecursive(head, value); }
